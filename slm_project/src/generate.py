"""generate.py — question -> two-stage retrieve -> constrained generate ->
faithfulness gate (one corrective regeneration, then flag) -> answer."""
import silence
...
for name in ("faiss", "faiss.loader", "sentence_transformers",
             "sentence_transformers.SentenceTransformer",
             "sentence_transformers.cross_encoder.CrossEncoder",
             "transformers", "transformers.tokenization_utils_base",
             "transformers.modeling_utils", "urllib3", "httpx"):
    logging.getLogger(name).setLevel(logging.ERROR)

warnings.filterwarnings("ignore")

# Kill tqdm progress bars globally (the "Batches: 2/2" spam)
try:
...
GROUNDED_TEMPERATURE = 0.3     # lower than Gemma default for factual QA
...
class QAResult:
    ...
    flagged: bool = False          # still unfaithful after regeneration
...
def load_pipeline(cfg: Config = CONFIG, model: str | None = None) -> Pipeline:
    ...
    client = OllamaClient(cfg, model=model)
    client.ensure_ready()
