"""raft_data.py — build a RAFT fine-tuning dataset from the module corpus.

RAFT (Zhang et al., arXiv:2403.10131): each example gives the model a
question plus a document set of 1 ORACLE (contains the answer) + N DISTRACTOR
chunks, with a chain-of-thought answer that cites verbatim from the oracle.
Paper-optimal recipe implemented here:
    * 4 distractors per example (cfg.raft_distractors),
    * oracle OMITTED in 20% of examples (cfg.raft_oracle_drop_frac) - for
      those, the target answer is the module refusal string, teaching the
      model to refuse when the context is all distractors,
    * additional pure refusal examples from off-topic questions,
    * every grounded answer ends with the [source, p.N] citation format the
      strict prompt demands.

Oracle selection is STRATIFIED BY SOURCE FILE, not uniform-random over
chunks. This corpus is ~6,300 chunks across ~300+ files, most of which are
tiny (1-4 chunks: transcripts, scripts, single-topic articles). Uniform
random sampling over chunks concentrates on files that happen to have more
chunks and starves the long tail - a 313-file corpus sampled 300 times left
286 files with ZERO oracle coverage in practice, including files gold
questions were later written against. Round-robining across source files
guarantees every file contributes at least one oracle before any file gets
a second, so fine-tuning coverage tracks the corpus's actual breadth rather
than its chunk-count skew.

The closed-corpus constraint holds: every question/answer is derived from the
module materials; the adapter learns grounding/citation/refusal BEHAVIOUR.

Output: raft/raft_train.jsonl with {"messages": [...]} chat rows - upload to
Colab and run colab_raft_finetune.ipynb. Generation is resumable (appends,
skips already-used oracle chunk ids recorded in raft/used_ids.txt).

Run:  python raft_data.py [--n 300] [--model gemma3n:e4b]
"""
...
def generate(cfg: Config = CONFIG, n: int | None = None,
    ...
    with out_path.open("a", encoding="utf-8") as out, \
        ...
        # Refusal examples are added only when the oracle pool is
        # exhausted (i.e. this is the run that completes the dataset), not
        # on every resumed invocation - otherwise the refusal fraction
        # compounds across runs and silently drifts far above
        # cfg.raft_refusal_frac.
